...
This use case shows how to enforce OpenID Connect authentication

`cd` into the lab directory
```code
cd ~/NGINX-Gateway-Fabric-Lab/labs/12.openid-connect
```
...
cert-manager-84dbf9d5ff-7r89k              1/1     Running   0          30s
cert-manager-cainjector-567c6b47ff-fgwgz   1/1     Running   0          30s
cert-manager-webhook-7cc5c588cb-slpln      1/1     Running   0          30s
```
...
```code
kubectl apply -f 0.clusterissuer.yaml
```
...
```code
kubectl apply -f 1.certificates.yaml
```

Deploy Keycloak. Keycloak must serve HTTPS because NGINX connects to it over TLS for token exchange. The keycloak-tls-cert Secret was created by cert-manager in the previous step and is mounted into the Keycloak container.
Keycloak is configured with a realm named `nginx-gateway`, a client with ID `nginx-gateway-coffee` and secret `oidc-coffee-client-secret`, and a test user with username `testuser` and password `testpassword`
```code
kubectl apply -f 2.keycloak.yaml
```
...
```code
NAME                        READY   STATUS    RESTARTS   AGE
keycloak-6548fbb9d7-pw6d9   1/1     Running   0          97s
```
...
```code
NAME                          READY   STATUS    RESTARTS   AGE
pod/coffee-654ddf664b-jbq44   1/1     Running   0          4s

NAME                 TYPE        CLUSTER-IP     EXTERNAL-IP   PORT(S)   AGE
service/coffee       ClusterIP   10.97.13.161   <none>        80/TCP    4s
service/kubernetes   ClusterIP   10.96.0.1      <none>        443/TCP   646d

NAME                     READY   UP-TO-DATE   AVAILABLE   AGE
deployment.apps/coffee   1/1     1            1           4s

NAME                                DESIRED   CURRENT   READY   AGE
replicaset.apps/coffee-654ddf664b   1         1         1       4s
```
